  ...
  return (
    <div className="w-full lg:w-[380px] shrink-0 flex flex-col gap-3.5">
      ...
      <div className={`p-4 md:p-5 rounded-xl border transition-all duration-300 shadow-md ${
        isValid 
          ? 'bg-[#ffffff] border-[#b8d4e3]' 
          : 'bg-red-50 border-red-200'
      }`}>
        <div className="flex flex-col h-full">
          <div className="flex items-center justify-between mb-3.5">
            <div className="flex items-center gap-2">
              <span className={`w-2 h-2 rounded-full ${isValid ? 'bg-cyan-600' : 'bg-red-400'}`}></span>
              <h3 className="text-xs font-mono font-bold text-[#334155] uppercase tracking-wider">
                Panel Inventory Summary
              </h3>
            </div>
            <span className={`text-xs font-mono font-medium px-2 py-2 rounded-md border ${
              isValid 
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex justify-between items-end bg-[#161a25] p-3 rounded-lg border border-[#a3c9db]">
              <div className="flex flex-col">
                ...
                <span className="text-2xl md:text-3xl font-bold text-[#0f172a] font-mono tracking-tight leading-none mt-1">
                  {data.fullRostrumsCount}
                </span>
              </div>
              <div className="text-xs text-[#64748b] font-mono font-semibold uppercase">Units</div>
            </div>
            ...
            <div className="flex justify-between items-end bg-[#161a25] p-3 rounded-lg border border-[#a3c9db]">
              <div className="flex flex-col">
                ...
                <span className={`text-2xl md:text-3xl font-bold font-mono tracking-tight leading-none mt-1 ${
                  data.halfRostrumsCount > 0 ? 'text-cyan-600' : 'text-[#64748b]'
                }`}>
          ...
          <div className="mt-3.5 pt-3 border-t border-[#b8d4e3] flex justify-between items-center text-xs font-mono">
            <span className="text-xs text-[#64748b] uppercase tracking-wider">Solver Metric</span>
            <span className="text-xs text-[#475569] font-medium">Standard Kwikstage Bays</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-2">
        <h4 className="text-xs font-mono font-semibold text-[#475569] uppercase tracking-wider px-1 flex items-center gap-1.5">
...
const StatCard: React.FC<{ label: string; value: string | number; unit: string; highlight?: boolean }> = ({
  ...
  >
    ...
    <div className="text-sm md:text-base font-bold text-[#0f172a] font-mono leading-none flex items-baseline gap-1">
      {value}
      <span className="text-xs text-[#64748b] font-normal">{unit}</span>
    </div>
  </div>
);
